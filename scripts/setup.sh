...
SEND_RESULT=$(curl -sf -X POST "http://localhost:8010/api/send?text=Setup+completo&reference_key=setup-test")
...
sleep 1
EDIT_RESULT=$(curl -sf -X PUT "http://localhost:8010/api/edit?reference_key=setup-test&text=Setup+completo+%E2%9C%85+editado")
echo "✅ Edição: $EDIT_RESULT"

# 9. Testar health
echo ""
echo "💚 Health check..."
HEALTH=$(curl -sf "http://localhost:8010/health")
echo "✅ Health: $HEALTH"
